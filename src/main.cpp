...
int main(int argc, char **argv)
{
        ...
        for (int i = 0; i < settings.numPasswords; i++)
        {
                ...
                std::vector<std::string> words = loadWordsFromDictionary(settings.language, indexes,
                                                                         settings.verbose, settings.binary);
                if (settings.style)
                {
                ...
                if (combined.size() + settings.numWords - 1 < settings.targetLength && settings.targetLength != -1)
                {
                        ...
                        if (settings.verbose)
                        {
                                std::cout << "Password target length: " << settings.targetLength
                                          << " is not greater than generated length: "
                                          << combined.size() + settings.numWords - 1 << ", regenerating\n";
                        }
                }
                else
                {
